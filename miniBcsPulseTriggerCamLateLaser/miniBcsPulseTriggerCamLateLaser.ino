...
//=== TASK VARIABLES ===
//======================
boolean trialOff = false; // trial status indicator
boolean laserOn = false;  // laser status indicator
boolean turnLaserOn = false; // laser switch to be used for each trial in a random manner 
...
unsigned long camOnTime = 0;  // time when the cam trigger goes on 
...
int laserDelay = 3000;  // dur to keep the laser off after the cam trigger onset  
  ...
  switch (pulseState) {
    case 0: // in this state, turn on the trigger pulse
      if ( trialOff == false ) { // trial offset TTL
        ...
        camOnTime = millis(); // cam trigger onset time
        turnLaserOn = random(1,randLaserEvery+1)>(randLaserEvery-1); 
