...
void eot_parser() {
  ...
  String data = trialData[0] + "," + trialData[1] + "," + trialData[2] + "," + trialData[3] + "," + trialData[4] + "," + trialData[5] + "," + trialData[6]+ ","+ paramWave[8]+ ","+ variable + "," + rightleft;
  output.println(data);
  output.flush();
...
void eot_parameter() {
  ...
  blockType = floor(count/50); // each block comprises 40 trials by default
  
  if (blockType>=3) {  // terminate the session once four blocks completed 
    // session is complete
    println(blockType);
    ...
    if (rand <= stimPerc & stimSession == 1 & blockType ==1){ // stim logic for the next trial, only to activate stim in the 2nd block (high threshold block)
    stimTrial = 1; // stim
    } else {
    stimTrial = 0; // no stim
    }
