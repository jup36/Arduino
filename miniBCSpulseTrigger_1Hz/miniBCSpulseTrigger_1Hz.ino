...
int sampleFreq = 1; // video sampling frequency (default: 300 Hz)
...
void setup(void) {
  ...
  analogWriteFrequency(PIN21, sampleFreq); // Teensy pin 20 set to 300 Hz
  ...
  dutyCycle  = 256/2; // duty cycle 256 equals to 100% duty cycle
  lastMillis = micros();
} // end of set up
  ...
  switch (pulseState) {
    case 0: // in this state, turn on the trigger pulse
      if ( trialOffset == false ) { // trial offset TTL
        analogWrite(PIN21, dutyCycle); // use pin 20, duty cycle 77/256*100 = 30%
    ...
    case 2: // in this stae, turn off the trigger pulses after passage of trigOffDelay (e.g. 1000ms)
      if ( time > trialOffsetTime + trigOffDelay) {
        analogWrite(PIN21, 0); // turn off the trigger pulse or digitalWrite(20, LOW);
    ...
    case 4: // idle state; 
      analogWrite(PIN21, 0); // just idle in this state until reactivated by push button 2;
      digitalWrite(DIOports[1], LOW);
      break;
  }
}
